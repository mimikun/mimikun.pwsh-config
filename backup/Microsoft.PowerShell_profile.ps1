...
if ($cond) {
    ...
    if ( ($isExistStarship) -and ($useShellPrompt -eq "starship") ) {
        ...
        $starship_completion = starship completions power-shell |
    ...
    # intelli-shell setup
    Get-Command intelli-shell -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# intelli-shell setup" >> $env:CACHE_PROFILE
        $intelli_shell_setup = intelli-shell init powershell
        Write-Output $intelli_shell_setup >> $env:CACHE_PROFILE
    }

    # ghtkn setup
    Get-Command ghtkn -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# ghtkn setup" >> $env:CACHE_PROFILE
        $ghtkn_setup = ghtkn completion pwsh
        Write-Output $ghtkn_setup >> $env:CACHE_PROFILE
    }

    # tod completion
    #Get-Command tod -ErrorAction Ignore | Out-Null
    #$res = $?
    #if ($res) {
    #    Write-Output "# cache tod completion" >> $env:CACHE_PROFILE
    #    $tod_completion = tod shell completions power-shell |
    #        ForEach-Object { $_ -replace $namespace1, "" } |
    #        ForEach-Object { $_ -replace $namespace2, "" }
    #    Write-Output $tod_completion >> $env:CACHE_PROFILE
    #}

    # dotter completion
    Get-Command dotter -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# cache dotter completion" >> $env:CACHE_PROFILE
        $dotter_completion = dotter gen-completions --shell powershell |
            ForEach-Object { $_ -replace $namespace1, "" } |
            ForEach-Object { $_ -replace $namespace2, "" }
        Write-Output $dotter_completion >> $env:CACHE_PROFILE
    }

    # ghciwatch completion
    Get-Command ghciwatch -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# cache ghciwatch completion" >> $env:CACHE_PROFILE
        $ghciwatch_completion = ghciwatch --completions powershell |
            ForEach-Object { $_ -replace $namespace1, "" } |
            ForEach-Object { $_ -replace $namespace2, "" }
        Write-Output $ghciwatch_completion >> $env:CACHE_PROFILE
    }

    # tombi completion
    Get-Command tombi -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# cache tombi completion" >> $env:CACHE_PROFILE
        $tombi_completion = tombi completion powershell |
            ForEach-Object { $_ -replace $namespace1, "" } |
            ForEach-Object { $_ -replace $namespace2, "" }
        Write-Output $tombi_completion >> $env:CACHE_PROFILE
    }

    # crush setup
    Get-Command crush -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# crush setup" >> $env:CACHE_PROFILE
        $crush_setup = crush completion powershell
        Write-Output $crush_setup >> $env:CACHE_PROFILE
    }

    # jump setup
    Get-Command jump -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# jump setup" >> $env:CACHE_PROFILE
        $jump_setup = jump shell pwsh
        Write-Output $jump_setup >> $env:CACHE_PROFILE
    }

    # ov completion
    Get-Command ov -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# ov completion" >> $env:CACHE_PROFILE
        $ov_completion = ov --completion powershell
        Write-Output $ov_completion >> $env:CACHE_PROFILE
    }

    # ty completion
    Get-Command ty -ErrorAction Ignore | Out-Null
    $res = $?
    if ($res) {
        Write-Output "# cache ty completion" >> $env:CACHE_PROFILE
        $ty_completion = ty generate-shell-completion powershell |
            ForEach-Object { $_ -replace $namespace1, "" } |
            ForEach-Object { $_ -replace $namespace2, "" }
        Write-Output $ty_completion >> $env:CACHE_PROFILE
    }
}
